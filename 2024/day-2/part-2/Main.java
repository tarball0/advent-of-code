...
public class Main {
	...
	public static boolean isSafe(int[] level) {
		...
		for (int i = 0; i < level.length; i++) {
			if (canBeValidByRemoving(level, i)) {
				return true;
			}
		}
		return false;
	...
	public static boolean canBeValidByRemoving(int[] level, int indexToRemove) {
		int[] temp = new int[level.length - 1];
		for (int i = 0, j = 0; i < level.length; i++) {
			if (i != indexToRemove) {
				temp[j++] = level[i];
			}
		}
		return isAscending(temp) || isDescending(temp);
	}
